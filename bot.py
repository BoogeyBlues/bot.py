...
# Position sizing — auto-scales with capital
TRADE_PCT   = float(os.environ.get("TRADE_PCT",   "18"))   # % of capital per trade
MIN_TRADE   = float(os.environ.get("MIN_TRADE",   "5"))    # floor
MAX_TRADE   = float(os.environ.get("MAX_TRADE",   "500"))  # ceiling (raise as you grow)
...
def check_milestones():
    with capital_lock:
        cap = capital
    for m in MILESTONES:
        if cap >= m and m not in milestones_hit:
            milestones_hit.add(m)
            next_trade = trade_size()
            log("ok", f"MILESTONE ${m:,}! Trade size now ${next_trade:.2f} | Next target: ${next((x for x in MILESTONES if x > m), '100k')}")
...
def load_learn():
    try:
        ...
        return {"history": [], "stats": {}, "adjustments": []}

def save_learn(state):
    try:
...
def record_trade(entry):
    with learn_lock:
        ...
        save_learn(state)
    if len(load_learn()["history"]) % 20 == 0:
        auto_tune()

def auto_tune():
    global BOND_ENTRY_MIN, BOND_ENTRY_MAX, MIN_REPLIES, BOND_TP, SPIKE_TP_PCT
    with learn_lock:
        state = load_learn()
    ...
    recent = history[-40:]
    by_strat  = defaultdict(lambda: {"wins": 0, "total": 0, "pnl": 0.0})
    by_hour   = defaultdict(lambda: {"wins": 0, "total": 0})
    by_reply  = defaultdict(lambda: {"wins": 0, "total": 0})
    win_bonds = []
    all_bonds = []

    for t in recent:
        s   = t.get("strategy", "bond")
        win = t.get("pnl", 0) > 0
        by_strat[s]["total"] += 1
        ...
        if win:
            by_hour[h]["wins"] += 1
        r_bucket = (t.get("replies", 0) // 10) * 10
        by_reply[r_bucket]["total"] += 1
        if win:
            by_reply[r_bucket]["wins"] += 1
        if t.get("strategy") == "bond" and "bond_entry" in t:
            all_bonds.append(t["bond_entry"])
            if win:
                win_bonds.append(t["bond_entry"])

    adjustments = []

    # Shift bond entry range toward winning entries
    if len(win_bonds) >= 3:
        avg = sum(win_bonds) / len(win_bonds)
        new_min = max(50.0, round(avg - 3, 1))
        new_max = min(75.0, round(avg + 3, 1))
        if abs(new_min - BOND_ENTRY_MIN) >= 1 or abs(new_max - BOND_ENTRY_MAX) >= 1:
            BOND_ENTRY_MIN, BOND_ENTRY_MAX = new_min, new_max
            adjustments.append(f"Bond entry {new_min}-{new_max}% (avg win entry was {avg:.1f}%)")

    # If spike is outperforming bond, increase spike TP target
    bond_wr  = by_strat["bond"]["wins"] / max(by_strat["bond"]["total"], 1)
    spike_wr = by_strat["spike"]["wins"] / max(by_strat["spike"]["total"], 1)
    if spike_wr > bond_wr + 0.2 and by_strat["spike"]["total"] >= 5:
        new_tp = min(60.0, SPIKE_TP_PCT + 5)
        if new_tp != SPIKE_TP_PCT:
            SPIKE_TP_PCT = new_tp
            adjustments.append(f"Spike TP raised to {SPIKE_TP_PCT}% (spike win rate {spike_wr:.0%} > bond {bond_wr:.0%})")

    # Best reply bucket
    best_reply_bucket = max(by_reply.keys(),
        key=lambda k: by_reply[k]["wins"] / max(by_reply[k]["total"], 1)
        if by_reply[k]["total"] >= 3 else 0, default=None)
    if best_reply_bucket is not None and best_reply_bucket > MIN_REPLIES:
        new_min_r = max(5, best_reply_bucket - 5)
        if new_min_r != MIN_REPLIES:
            MIN_REPLIES = new_min_r
            adjustments.append(f"MIN_REPLIES → {MIN_REPLIES} (best wins in {best_reply_bucket}+ bucket)")

    best_hours = sorted(
        [h for h, s in by_hour.items() if s["total"] >= 2 and s["wins"] / s["total"] >= 0.5],
        key=lambda h: by_hour[h]["wins"] / max(by_hour[h]["total"], 1), reverse=True
    )[:5]

    stats = {
        "total_trades":  len(history),
        "recent_trades": len(recent),
        "by_strategy":   {k: {**v, "win_rate": round(v["wins"]/max(v["total"],1)*100,1)} for k, v in by_strat.items()},
        ...
        "current_params": {
            ...
            "trade_pct":   f"{TRADE_PCT}% of capital",
        },
        "tuned_at": time.strftime("%H:%M:%S"),
    }

    with learn_lock:
        state2 = load_learn()
        state2["stats"]       = stats
        state2["adjustments"] = adjustments
        save_learn(state2)

    if adjustments:
        for a in adjustments:
            log("learn", f"AUTO-TUNE: {a}")
    else:
        log("learn", f"Auto-tune complete ({len(recent)} trades) — no changes needed")
...
def check_bundle(mint):
    ...
    try:
        ...
        disq  = any("mint" in r or "freeze" in r for r in risks)
        bund  = any("bundle" in r or "insider" in r or "sniper" in r for r in risks)
        bundle_cache[mint] = {"bundled": bund, "top10": top10, "disqualified": disq, "ts": time.time()}
        return bund, top10, disq
    except:
...
def execute_buy(mint, symbol, amount):
    if PAPER_MODE:
        log("ok", f"[PAPER] Buy ${amount:.2f}", symbol)
        return "PAPER_TX"
    try:
        sol_amount = round(amount / get_sol_price(), 6)
        res = _session.post(
            PUMPPORTAL,
        ...
        if sig and len(sig) > 10:
            log("ok", f"Bought ${amount:.2f} | {sig[:20]}...", symbol)
            return sig
        return None
    except Exception as e:
...
def enter_trade(coin, strategy):
    ...
    with capital_lock:
        if capital < amount:
            log("warn", f"Low capital ${capital:.2f} < ${amount:.2f}")
            return False
    ...
    sl_pct   = SPIKE_SL_PCT if strategy == "spike" else BOND_SL_PCT
    max_secs = SPIKE_MAX_SECS if strategy == "spike" else BOND_MAX_SECS
    sl_price = price * (1 - sl_pct / 100)

    if strategy == "bond":
        log("ok", f"[BOND] IN ${amount:.2f} | bond:{bond:.1f}% | TP:>{BOND_TP}% | SL:-{BOND_SL_PCT}%", symbol)
    elif strategy == "bundle":
        log("ok", f"[BUNDLE] IN ${amount:.2f} | bond:{bond:.1f}% | TP:>{BUNDLE_RIDE_TP}% | SL:-{BOND_SL_PCT}%", symbol)
    else:
        log("ok", f"[SPIKE] IN ${amount:.2f} | 1h:+{coin.get('change1h',0):.0f}% | TP:+{SPIKE_TP_PCT}% | SL:-{SPIKE_SL_PCT}%", symbol)
    ...
    with trades_lock:
        open_trades[mint] = {
            ...
            "sl":              sl_price,
            "amount":          amount,
            "tokens":          amount / price if price > 0 else 0,
            "opened_at":       time.time(),
...
def exit_trade(mint, price_now, reason, bond_now=None):
    ...
    log("ok" if pnl >= 0 else "err",
        f"{emoji} [{strategy.upper()}] {reason} | {'+' if pnl>=0 else ''}${pnl:.2f} | {hold_s:.0f}s{bond_str} | Cap:${capital:.2f} | Trade:${trade_size():.2f}",
        trade["symbol"])
    ...
    record = {
        ...
        "bond_entry": trade.get("bond_entry", 0),
        "replies":    trade.get("replies", 0),
    ...
    while True:
        ...
        for mint in mints:
            ...
            if strategy in ("bond", "bundle"):
                ...
                log("info", f"[{strategy.upper()}] bond:{bond:.1f}% high:{bond_high:.1f}% → {tp_bond}% | {move:+.1f}% | {hold_s:.0f}s", trade["symbol"])
                if bond >= tp_bond:
                    exit_trade(mint, price, "TP", bond)
                elif price <= trade["sl"]:
                    exit_trade(mint, price, "SL", bond)
                elif hold_s >= trade["max_secs"]:
                    exit_trade(mint, price, "TIME", bond)
            else:
                tp_price = trade["entry"] * (1 + SPIKE_TP_PCT / 100)
                log("info", f"[SPIKE] {move:+.1f}% → +{SPIKE_TP_PCT}% | bond:{bond:.1f}% high:{bond_high:.1f}% | {hold_s:.0f}s", trade["symbol"])
                if price >= tp_price:
...
def scanner_loop():
    ...
    load_learn()
    log("ok", "=" * 55)
    log("ok", "Pump.fun Sniper — PROGRESSIVE COMPOUNDER")
    log("ok", f"Capital: ${capital:.2f} | Trade: ${trade_size():.2f} ({TRADE_PCT}% of capital)")
    log("ok", f"[BOND]   {BOND_ENTRY_MIN}-{BOND_ENTRY_MAX}% entry | TP:>{BOND_TP}% | SL:-{BOND_SL_PCT}%")
    log("ok", f"[SPIKE]  Age:>{SPIKE_MIN_AGE_H}h | 1h:>{SPIKE_MIN_1H}% | TP:+{SPIKE_TP_PCT}% | SL:-{SPIKE_SL_PCT}%")
    log("ok", f"[SLIP]   >{SLIP_TRIGGER}%→<{SLIP_DROP_TO}% = exit in {SLIP_WAIT_SECS}s | Sharp:{SHARP_DROP_PCT}%/3s = instant")
    log("ok", f"[BUNDLE] {BUNDLE_MODE.upper()} | Required: Twitter+Telegram+{MIN_REPLIES}+ replies")
    log("ok", f"Mode: {'PAPER' if PAPER_MODE else 'LIVE'} | Goal: $54 → $100k")
    log("ok", "=" * 55)
    ...
    while scan_active:
        try:
            ...
            for coin in coins:
                ...
                is_bundled, top10, disq = check_bundle(mint)
                if disq:
                    continue
                if is_bundled:
                    if BUNDLE_MODE == "ride" and BOND_ENTRY_MIN <= bond <= 75:
                        bundle_cands.append(coin)
                        log("ok", f"[BUNDLE] bond:{bond:.1f}% top10:{top10:.0f}% replies:{coin['replies']}", coin["symbol"])
                    else:
                        log("info", f"Bundle skipped ({BUNDLE_MODE})", coin["symbol"])
                    continue
            ...
            if not bond_cands and not spike_cands and not bundle_cands:
                log("info", f"Scanning {len(coins)} coins | Cap:${capital:.2f} | Trade:${trade_size():.2f} | {num_open}/{MAX_OPEN} open")
...
def home():
    with trades_lock:
        n = len(open_trades)
    return f"Pump Sniper | ${capital:.2f} | Trade:${trade_size():.2f} | Open:{n}/{MAX_OPEN} | {'PAPER' if PAPER_MODE else 'LIVE'}", 200
...
def status():
    ...
    with trades_lock:
        n = len(open_trades)
    next_m = next((m for m in MILESTONES if m > capital), None)
    return jsonify({
        ...
        "milestones_hit": sorted(list(milestones_hit)),
...
def learn_stats():
    ...
    if not stats:
        ...
        return jsonify({"msg": f"Need 15 trades to tune. Have {total} so far."})
