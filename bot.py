...
# ── PRICE FETCH ──────────────────────────────────────────────────
def get_sol_price():
    """Get SOL/USD price for USDC to SOL conversion."""
    try:
        res = requests.get(
...
def get_prices_bulk(retries=3):
    """Fetch all token prices in one CoinGecko call."""
    ...
    for attempt in range(retries):
        try:
            ...
            if prices:
                return prices
            log("warn", "Empty price response")
            time.sleep(30)
        except Exception as e:
            log("warn", f"Price error: {e}")
            time.sleep(15)
    return {}
...
def execute_swap(amount_usdc, output_mint, symbol):
    ...
    Signs locally with Phantom private key.
...
def scanner_loop():
    ...
    log("info", f"Warming up ({EMA_SLOW_LEN} samples, 45s apart)...")
